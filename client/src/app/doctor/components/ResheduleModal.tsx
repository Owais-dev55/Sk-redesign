...
export default function RescheduleModal({ appointmentId, onClose, refetch }: RescheduleModalProps) {
  ...
  try {
    setLoading(true)
    const res = await fetch(`http://localhost:3000/api/appointments/doctor/${appointmentId}`, {
      method: "PATCH",
      headers: {
      ...
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-sm sm:max-w-md relative">
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 mt-4">
          ...
          >
            {loading ? (
              <>
                <FaSpinner className="animate-spin mr-2 h-4 w-4" /> Rescheduling...
              </>
            ) : (
              "Confirm Reschedule"
            )}
